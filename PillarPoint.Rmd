...
p_patch <- p1 / (p2 & theme(plot.margin=ggplot2::margin(0,0,0,0, "pt"))) & ylab(NULL) & theme(plot.background = element_rect(fill = "transparent", colour = NA_character_))
...
#Latitude + Longitude
lat_lon = do.call(paste, list(NCBI_export$Latitude, "N", gsub("-", "", NCBI_export$Longitude), "W", sep=" "))
...
NCBI_export$collection_date_local <- lubridate::ymd_hm(paste(NCBI_export$Date, "T", NCBI_export$Time), tz = "US/Pacific")

NCBI_export$collection_date <- with_tz(NCBI_export$collection_date_local, "Zulu")
...
#Rename columns
NCBI_export = NCBI_export[c("Sample", "Sample_Name", "Type", "Location", "T", "S", "CRS", "lat_lon", "collection_date", "collection_date_local")]
NCBI_export = NCBI_export %>% rename(sample_name = Sample, sample_title = Sample_Name, sample_control = Type, salinity = S, temperature = T, geodeticDatum = CRS)
...
if (!dir.exists("Analysis Products/Processed eDNA/NCBI")) {dir.create("Analysis Products/Processed eDNA/NCBI", recursive = TRUE)}
write.csv(NCBI_export, "Analysis Products/Processed eDNA/NCBI/BioSample_Attributes.csv", row.names=FALSE)
write.csv(SRA_metadata, "Analysis Products/Processed eDNA/NCBI/SRA_metadata.csv", row.names=FALSE)
...
defaultValues = defaultValues %>% 
  ...
  add_row(term = "samplingProtocol", value = "dx.doi.org/10.17504/protocols.io.bp2l69y7klqe/v2") %>%
  ...
  add_row(term = "samp_collec_method", value = "Covidien single-use enteral feeding pouch: dx.doi.org/10.17504/protocols.io.bp2l69y7klqe/v2") %>%
  add_row(term = "samp_mat_process", value = "Gravity filtered through Sterivex (0.22-µm): dx.doi.org/10.17504/protocols.io.bp2l69y7klqe/v2") %>%
  ...
  add_row(term = "nucl_acid_ext", value = "dx.doi.org/10.17504/protocols.io.ewov1qyyygr2/v1") %>%
  add_row(term = "nucl_acid_amp", value = "dx.doi.org/10.17504/protocols.io.dm6gp3wpdvzp/v1") %>%
...
GBIF_export$metadata$verbatimEventDate <- lubridate::ymd_hm(paste(GBIF_export$metadata$Date, "T", GBIF_export$metadata$Time), tz = "US/Pacific")

GBIF_export$metadata$eventDate <- with_tz(GBIF_export$metadata$verbatimEventDate, "Zulu")
...
names(GBIF_export$metadata)[names(GBIF_export$metadata) == 'Sample'] <- 'id'

col_order = c("id", "Sample_Name", "Location", "Latitude", "Longitude", "CRS", "eventDate", "verbatimEventDate", "T", "S")
...
if (dir.exists("Analysis Products/Processed eDNA/NCBI/NCBI Upload Record")) {
  SRA = read.delim("Analysis Products/Processed eDNA/NCBI/NCBI Upload Record/metadata-14288727-processed-ok.tsv", sep="\t")
  SRA = SRA[c("accession", "biosample_accession", "sample_name")]
  SRA$biosample_accession=paste0("https://www.ncbi.nlm.nih.gov/biosample/?term=", SRA$biosample_accession)
...
GBIF_export$metadata = GBIF_export$metadata %>% rename(associatedSequences = accession, materialSampleID = biosample_accession, temperature = T, salinity = S)
...
sequences$sequence_unmerged = paste0("[UNMERGED SEQUENCE COMBINATION] F: ", sequences$sequencesF, "; R: ", sequences$sequencesR)

sequences <- sequences %>% mutate(sequence = ifelse(sequence=="", sequence_unmerged, sequence))
...
if (!dir.exists("Analysis Products/Processed eDNA/GBIF")) {dir.create("Analysis Products/Processed eDNA/GBIF", recursive = TRUE)}
write.csv(GBIF_export$abund, "Analysis Products/Processed eDNA/GBIF/OTUtable.csv", row.names=TRUE)
write.csv(GBIF_export$tax, "Analysis Products/Processed eDNA/GBIF/taxa.csv", row.names=TRUE)
write.csv(GBIF_export$metadata, "Analysis Products/Processed eDNA/GBIF/samples.csv", row.names=FALSE)
write.csv(defaultValues, "Analysis Products/Processed eDNA/GBIF/defaultValues.csv", row.names=FALSE)
...
if (!dir.exists("Analysis Products/Processed eDNA/Manuscript Analysis")) {dir.create("Analysis Products/Processed eDNA/Manuscript Analysis", recursive = TRUE)}
...
combined = (plot_spacer() + p_patch + plot_spacer() + plot_layout(widths = c(3,50,1.2)))/(plot_spacer())/(plot_spacer() + sample_schema + plot_layout(widths = c(1,50))) + plot_layout(heights = c(4, -.05, 5)) & theme(plot.margin=ggplot2::margin(0,0,0,0, "pt"))
...
if(file.exists("Data/BetweenPacificTides_TextForSearching.pdf") & file.exists("Data/BetweenPacificTides_TOC.pdf")) {

txt <-pdf_text("Data/BetweenPacificTides_TextForSearching.pdf")
toc <-pdf_text("Data/BetweenPacificTides_TOC.pdf")

## POTENTIAL DATA FRAME SAVING MOMENT (uncleaned full text in txt and toc)
...
###POTENTIAL DATA FRAME SAVING MOMENT (post initial cleaning of txt and toc)
...
###POTENTIAL DATA FRAME SAVING MOMENT (txt still contains full text, toc is basically unrecognizable from original)
...
write.csv(final_1, "Analysis Products/BPT_Analysis_Intermediate.csv", row.names=FALSE)
...
contingency_table = condense_no_pages %>% group_by(Site) %>% summarise(High = sum(Zone_1 == TRUE | Zone_2 == TRUE | Zone_3 == TRUE), Low = sum(Zone_1 == FALSE & Zone_2 == FALSE & Zone_3 == FALSE))
...
```{r Ecological Significance Table, message = FALSE, warning = FALSE, results = FALSE}

#Create table showcasing results
...
if (!dir.exists("Figures/PowerPoint")) {dir.create("Figures/PowerPoint", recursive = TRUE)}
