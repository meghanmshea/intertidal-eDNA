...
###Figure 3--Continued###
#Finish the bottom portion of Figure 3 not that data has been loaded
...
save_pdf_png(plot = p_patch/sample_schema, path = path, w = 169, h = 180, u = "mm", bg = "transparent", dpi = 600)
...
percentages = GBIF_summary %>% group_by(across(PillarPointOccurrence:NotInGBIF)) %>% summarise(count=n()) %>% mutate(per = count/sum(count))
percentages 
...
*Figure 4 version for PowerPoint*
```{r Phylogenetic Tree for PowerPoint, warning = FALSE, class.source = 'fold-hide'}
...
```{r NMDS & PAM, message=FALSE, cache = TRUE, warning = FALSE, results = FALSE}
...
```{r amp_nocontrols,  class.source = 'fold-hide'}
