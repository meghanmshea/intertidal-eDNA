...
overlap_table = list(ASV = intersect,
...
data_physeq = amp_to_phyloseq(data_amp, NA, NA, NA)
  ...
  range_function <- function(species){
    ...
    for (i in species) {
      ...
      for (l in limits) {
        ...
        bb_occ = bbox(cbind(occ.df$longitude, occ.df$latitude))
...
distribution = distribution_check(c(-122.50, 37.49, -122.49, 37.50), CC_shape, species_standardized, 5, FALSE)
...
summary = data.frame(Species = species_both$verbatim_name, 
...
summary %>% group_by(across(PillarPointOccurrence:NotInGBIF)) %>% summarise(count=n())

write.csv(summary, "Data/Analysis Products/RangeAnalysis.csv", row.names=TRUE)
```
