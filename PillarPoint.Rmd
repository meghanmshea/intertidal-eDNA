...
BM_packages <- c("phyloseq", 
                 "ggtreeExtra")

 BiocManager::install(setdiff(BM_packages, rownames(installed.packages())))
...
#most recent CRAN update for tidytree broke my phylogenetic tree code; see if I can troubleshoot at some point
if(!require("tidytree", quietly=TRUE))
  install_version("tidytree", version = "0.4.2")

packages = append(packages, c("phyloseq", "ampvis2", "ggtree", "ggtreeExtra", "tidytree"))
...
We collected 1 L surface samples from each location every 30 minutes for the duration of time the intertidal was exposed on 28 January 2022, using single-use enteral feeding pouches (Covidien, Dublin, Ireland). Sampling commenced at 11:30 am PST; the exact times of sample collection, in relation to the tide, are shown in Figure 3. Following the approach used by Gold et al. (2021b), we attached a sterile 0.22 $\mu$m pore size Sterivex cartridge (MilliporeSigma, Burlington, MA, USA) to the tubing of each feeding pouch, allowing samples to be immediately gravity filtered in the field. While gravity filtering (1-2 hours per sample), samples were shaded with an awning to prevent any degradation by sunlight (Andruszkiewicz et al., 2017). 


```{r Figure 3,  fig.width= 169, fig.height = 90}
...
ggsave(
  plot = p_patch/sample_schema,
  filename = "Figures/Figure3_full.png",
  device = "png",
  bg = "transparent",
  width = 169,
  height = 200,
...
known_summary = GBIF_summary %>% filter(!is.na(GBIFSpecies)) %>% summarise(count = sum(Range == TRUE & CCSOccurrence == TRUE))
...
Focused on just the `r samples` (`samples`) samples, we removed singletons and all `r overlap` (`overlap`) ASVs that overlapped with negative controls (Supplemental Information X), and we rarefied samples to the minimum number of reads of any sample, leaving `r processed_ASVs` (`processed_ASVs`) ASVs and `r processed_reads` (`processed_reads`) reads. `r sprintf("%0.1f%%", unassigned_ASVs[unassigned_ASVs$Kingdom == "",]$freq * 100)` (`sprintf("%0.1f%%", unassigned_ASVs[unassigned_ASVs$Kingdom == "",]$freq * 100)`) of ASVs were unassigned, and the remainder included representatives from `r phyla` (`phyla`) phyla, `r class` (`class`) classes, `r order` (`order`) orders, `r family` (`family`) families, `r genus` (`genus`) genera, and `r species` (`species`) species (Figure 4). A breakdown of the relative proportions of reads and ASVs assigned to each phyla can be found in Table 1. To ensure the accuracy of these taxonomic identifications, we confirmed that `r known_summary` (`known_summary`) of `r species` (`species`) identified species (`r sprintf("%0.1f%%", known_summary/species * 100)` `sprintf("%0.1f%%", known_summary/species * 100)`) have occurrence records in the California Current System (CCS) and known ranges that encompass Pillar Point. (Supplemental Information X).
...
```{r Phylogenetic Tree for PowerPoint, class.source = 'fold-hide'}
...
```{r Indicator Species Analysis, warning = FALSE}
...
path = "Figures/Figure6.png"
  ggsave(filename = path, plot = heatmap_taxa$plot, device = "png", width = 169, height = 120, units = "mm", dpi = 600)
...
```{r NMDS & PAM, message=FALSE, echo=FALSE, cache = TRUE, warning = FALSE, results = FALSE}
...
```{r Animated Figure for PowerPoint, message=FALSE, echo=FALSE, warning = FALSE, results = FALSE}
...
path = "Figures/Figure8.pdf"
  ggsave(filename = path, plot = g2.5, device = "pdf", width = 169, height = 120, units = "mm", dpi = 600)

```

#### 3.4.	Ecological Significance

```{r Ecological Significance, message = FALSE, warning = FALSE, results = FALSE}
