...
To improve reproducibility (e.g. Dickie et al., 2018; Shea et al., 2023), enable open data science (e.g. Fredston and Lowndes, 2024), and aid in the initiation of new eDNA biomonitoring projects, we have published detailed, step-by-step protocols for many of the methods described, including specific materials used, photographs, and additional methodological notes not possible to include here. See Shea and Boehm for sample collection and filtering ([2023b](https://dx.doi.org/10.17504/protocols.io.bp2l69y7klqe/v2)), for DNA extractions ([2023c](https://dx.doi.org/10.17504/protocols.io.ewov1qyyygr2/v1)), for PCR amplification ([2023d](https://dx.doi.org/10.17504/protocols.io.dm6gp3wpdvzp/v1)), and for shipping samples ([2023e](https://dx.doi.org/10.17504/protocols.io.dm6gp38jpvzp/v1)). Additionally, we have published all data and code for replicating our analyses via Dryad ([Shea & Boehm, 2024](https://doi.org/10.5061/dryad.pnvx0k6tf)), including FASTQ files and eDNA datasets (pre-processing & post-processing). Through Dryad and Zenodo, our modified Anacapa Container and scripts for bioinformatics ([Shea & Boehm, 2023a](https://zenodo.org/doi/10.5281/zenodo.8201140)), as well as a GitHUb repository including an R Markdown file that reproduces all methods & results detailed here ([Shea & Boehm, 2023f](https://zenodo.org/doi/10.5281/zenodo.8213050)) can also be accessed.
...
```{r Making NCBI Data Frames, warning = FALSE}
...
```{r Making GBIF Data Frames, warning = FALSE, cache = TRUE,  results = 'hide'}
...
defaultValues = defaultValues %>%
  ...
  add_row(term = "amplificationReactionVolumeUnit", value = "µl") %>%
...
#taxonConceptID
GBIF_export$tax$taxonConceptID = sub('.*\\_', '', GBIF_export$tax$Species)
GBIF_export$tax$taxonConceptID <- ifelse(GBIF_export$tax$taxonConceptID == "", sub('.*\\_', '', GBIF_export$tax$Genus),GBIF_export$tax$taxonConceptID)
GBIF_export$tax$taxonConceptID <- ifelse(GBIF_export$tax$taxonConceptID == "", sub('.*\\_', '', GBIF_export$tax$Family),GBIF_export$tax$taxonConceptID)
GBIF_export$tax$taxonConceptID <- ifelse(GBIF_export$tax$taxonConceptID == "", sub('.*\\_', '', GBIF_export$tax$Order),GBIF_export$tax$taxonConceptID)
GBIF_export$tax$taxonConceptID <- ifelse(GBIF_export$tax$taxonConceptID == "", sub('.*\\_', '', GBIF_export$tax$Class),GBIF_export$tax$taxonConceptID)
GBIF_export$tax$taxonConceptID <- ifelse(GBIF_export$tax$taxonConceptID == "", sub('.*\\_', '', GBIF_export$tax$Phylum),GBIF_export$tax$taxonConceptID)
GBIF_export$tax$taxonConceptID <- ifelse(GBIF_export$tax$taxonConceptID == "", sub('.*\\_', '', GBIF_export$tax$Kingdom),GBIF_export$tax$taxonConceptID)

GBIF_export$tax$taxonConceptID = paste0("NCBI:txid", GBIF_export$tax$taxonConceptID)
...
#Prepare to get WORMS Taxonomy
Taxonomy = GBIF_export$tax[c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species")]
Taxonomy = unique(Taxonomy[c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species")])
...
#tryCatch wrapper for wm_records_taxamatch
wm_records_taxamatch_fixed <-function(id) {
  tryCatch(
    {
      record = wm_records_taxamatch(id)
      return(record)
    },
...
#Function that checks if this is a valid AphiaID for a record
checkID = function(ncbi_id) {
  if(ncbi_id == "") {
    return(NULL)
  } else {
    
    record = wm_name2id_(ncbi_id)
    
    if(length(record) == 0) {
      record = wm_records_taxamatch_fixed(ncbi_id)
      if(is.null(record)) {
        return(NULL)
      } else {
        id = record[[1]]$AphiaID
...
##Get valid worrms taxonomy
for(i in 1:nrow(Taxonomy)) {
  id = Taxonomy$Species[i]
  print(id)
  aphia = checkID(id)
  
  if(!is.null(aphia)) {
   record = wm_record(aphia)
    if(!is.null(record$lsid)) {Taxonomy$lsid[i] = record$lsid}
    if(!is.null(record$rank)) {Taxonomy$rank[i] = record$rank}
    if(!is.null(record$scientificname)) {Taxonomy$scientificname[i] = record$scientificname}
    if(!is.null(record$kingdom)) {Taxonomy$kingdom[i] = record$kingdom}
    if(!is.null(record$phylum)) {Taxonomy$phylum[i] = record$phylum}
    if(!is.null(record$class)) {Taxonomy$class[i] = record$class}
    if(!is.null(record$order))  {Taxonomy$order[i] = record$order}
    if(!is.null(record$family)) {Taxonomy$family[i] = record$family}
    if(!is.null(record$genus)) {Taxonomy$genus[i] = record$genus}
    next
  } 
...
GBIF_export$tax = left_join(GBIF_export$tax, Taxonomy, by = c("Kingdom" = "Kingdom", "Phylum" = "Phylum", "Class"= "Class", "Order" = "Order", "Family" = "Family", "Genus" = "Genus", "Species" = "Species"))


#Capture original NCBI identification

GBIF_export$tax$verbatimIdentification = paste0("Kingdom: ", 
                                                GBIF_export$tax$Kingdom,
                                                " | Phylum: ",
                                                GBIF_export$tax$Phylum,
                                                " | Class: ",
                                                GBIF_export$tax$Class,
                                                " | Order: ",
                                                GBIF_export$tax$Order,
                                                " | Family: ",
                                                GBIF_export$tax$Family,
                                                " | Genus: ",
                                                GBIF_export$tax$Genus,
                                                " | Species: ",
                                                GBIF_export$tax$Species)



GBIF_export$tax$verbatimIdentification[GBIF_export$tax$verbatimIdentification == "Kingdom:  | Phylum:  | Class:  | Order:  | Family:  | Genus:  | Species: "] = NA
GBIF_export$tax$taxonConceptID[GBIF_export$tax$taxonConceptID == "NCBI:txid"] = NA
GBIF_export$tax$scientificname[GBIF_export$tax$scientificname == ""] = "incertae sedis"
GBIF_export$tax$lsid[GBIF_export$tax$scientificname == "incertae sedis"] = "urn:lsid:marinespecies.org:taxname:12"
GBIF_export$tax$rank[GBIF_export$tax$scientificname == "incertae sedis"] = "Kingdom"

row.names(GBIF_export$tax) = GBIF_export$tax$OTU
...
To further investigate whether differences in eDNA detections corresponded with underlying ecological gradients, we compared the unique and indicator taxa identified at each site to their ecological zonation in a highly regarded field guide to the Pacific intertidal, Between Pacific Tides (Ricketts et al., 1985). To account for potential variations in taxonomic names between Between Pacific Tides and the MIDORI2 reference database, we used the World Register of Marine Species (WoRMS) to identify all synonymized names for each unique and indicator taxon identified using the `taxize` (version `r packageVersion("taxize")` `packageVersion("taxize")`) (S. A. Chamberlain & Szöcs, 2013) package. Then, we searched Between Pacific Tides for all synonyms, both manually using the index and in R by extracting the text from a PDF of Between Pacific Tides using the `pdftools` package (version `r packageVersion("pdftools")` `packageVersion("pdftools")`) (Ooms, 2023). We compared the proportion of 
Zone 1 (uppermost horizon), Zone 2 (high intertidal), and Zone 3 (middle intertidal) species identified across each location using a chi-squared test with p-values computed via Monte Carlo simulation.
...
Using the Anacapa Toolkit, we identified `r total_ASVs` (`total_ASVs`) ASVs from `r total_reads`(`total_reads`) reads across `r samples` (`samples`) samples and `r controls` (`controls`) controls (positive PCR controls, negative field controls, negative extraction controls, and no-template negative PCR controls). All expected taxa were present in each positive PCR control, and `r MC_overlap` (`MC_overlap`). Before application of any decontamination steps, `r overlap` (`overlap`) ASVs were shared between the samples and negative field, extraction, and PCR controls.
...
![*Figure 3: Graph of tide predictions during field sampling on 28 January 2022, showing both the full tidal cycle (top) and the subset of the tidal cycle (bottom) during which samples were collected, with icons to indicate the sampling scheme*](`r paste0(path, "png")`)
...
Focused on just the `r samples` (`samples`) samples, we removed singletons and all `r overlap` (`overlap`) ASVs that overlapped with negative controls (Table S1), and we rarefied samples to the minimum number of reads of any sample, leaving `r processed_ASVs` (`processed_ASVs`) ASVs and `r processed_reads` (`processed_reads`) reads. `r sprintf("%0.1f%%", unassigned_ASVs[unassigned_ASVs$Kingdom == "",]$freq * 100)` (`sprintf("%0.1f%%", unassigned_ASVs[unassigned_ASVs$Kingdom == "",]$freq * 100)`) of ASVs were unassigned, and the remainder included representatives from `r unique["Phylum"]` (`unique["Phylum"]`) phyla, `r unique["Class"]` (`unique["Class"]`) classes, `r unique["Order"]` (`unique["Order"]`) orders, `r unique["Family"]` (`unique["Family"]`) families, `r unique["Genus"]` (`unique["Genus"]`) genera, and `r unique["Species"]` (`unique["Species"]`) species (Figure 4). A breakdown of the relative proportions of reads and ASVs assigned to each phyla can be found in Table 1. We confirmed that `r known_summary` (`known_summary`) of `r unique["Species"]` (`unique["Species"]`) identified species (`r sprintf("%0.1f%%", known_summary/unique["Species"] * 100)` `sprintf("%0.1f%%", known_summary/unique["Species"] * 100)`) have occurrence records in the California Current System (CCS) and known ranges that encompass Pillar Point (additional details in Appendix S1 and Table S2).
